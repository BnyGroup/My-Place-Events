...
<head>
	...
	<title>{{ forcompany() }} - {{--Order Confirmed--}}@lang('words_custom.mail_reject.order_confrimed')</title>
	<style type="text/css">
		@font-face {
		...
		<div class="mail-inner-content">
			...
			<div class="mail-header">
				...
				<div class="mail-header-title" >
					<h1>{{--Refund order--}}@lang('words_custom.mail_reject.refund_order') #{{ $userdata->order_id}}</h1>
				</div>
			</div>
			<div class="mail-body">
				<div class="body-title">
					<h4>
						Hi <span> {{ user_data($userdata->user_id)->fullname }} </span>, {{--Your refund is rejected--}} <b>@lang('words_custom.mail_reject.request_result')</b></span>
					</h4>
				...
				<div class="order-summary">
					...
					<div class="order-body">
						<p class="note">
							{{--Your--}}Votre <b>#{{ $userdata->order_id }}</b> {{--refund request is rejected for few some reason listed are below.--}}
							demande de remboursement est rejetée pour quelques-unes des raisons énumérées ci-dessous.
						</p>
						<p class="note">
							<label><b>{{--Reason--}}Raison :</b></label>
						...
						<p class="term-service">
	            			{{--Follow the --}}Suivez moi{{ siteSetting()->title }} <a href="" target="_blank" >{{--Terms of Service --}}Termes du Services</a> {{--and  --}}et<a href="" target="_blank" >{{--Privacy Policy--}}Politique de confidentialité</a>
	        			</p>
					</div>
					<div class="bg-down">
						...
						<tr>
							...
							<td>
								...
								<div class="login-link">
									<a href="{{ route('user.signup') }}" target="_blank">{{--Log in--}}Connectez-vous</a> {{--to access tickets and manage your orders --}}pour acceder au billet et gérer vos commandes
								</div>
							</td>
						</tr>
					</table>
			...
			<div class="mail-footer">
				<p>{{--This email was sent to --}}Ce email a été envoyé à{{ siteSetting()->site_email }} <br/>
				 Copyright © {{ date('Y') }} {{ siteSetting()->title }}. {{--All rights reserved--}}Tous droits réservés.  </p>
			</div>
		</div>
	</div>
</body>
