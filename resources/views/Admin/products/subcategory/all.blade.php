@extends($AdminTheme)
@section('title')
    {{__('Sous-catégories de Produits')}}
@endsection
    ...
    <div class="col-lg-12 col-ml-12 padding-bottom-30">
        <div class="row">
            ...
            <div class="col-lg-7 mt-5">
                <div class="card">
                    <div class="card-body">
                        <h4 class="header-title">{{__('Toutes les sous-catégories de produits')}}</h4>
                        ...
                        <div class="table-wrap table-responsive">
                            <table class="table table-default">
                                <thead>
                                    <th class="no-sort sorting_disabled" rowspan="1" colspan="1" aria-label="" style="width: 31.1406px;">
										<div class="mark-all-checkbox">
											<input type="checkbox" class="all-checkbox">
										</div>
									</th>
                                    <th>{{__('ID')}}</th>
                                    <th>{{__('Nom de la sous catégorie')}}</th>
                                    <th>{{__('Statut')}}</th>
                                    <th>{{__('Image')}}</th>
                                    <th>{{__('Action')}}</th>
                                </thead>
                                    ...
                                    <tr>
										<td>
											<div class="bulk-checkbox-wrapper">
												<input type="checkbox" class="bulk-checkbox" name="bulk_delete[]" value="{{$subcategory->id}}">
											</div>
										</td>
                                        <td>{{$loop->iteration}}</td>
                                        <td>{{$subcategory->title}}</td>
                                        <td>
                                           <?php if($subcategory->status=='publish'){ echo'<span class="alert alert-success">Publié</span>'; }
                                        ...
                                        <td>
											...
											<form method="post" action="{{ route('admin.products.subcategory.delete', $subcategory->id) }}" class="d-none">
            ...
            <div class="col-lg-5 mt-5">
                <div class="card">
                    <div class="card-body">
                        <h4 class="header-title">{{__('Ajouter une nouvelle sous-catégorie')}}</h4>
                        <form action="{{route('admin.products.subcategory.new')}}" method="post" enctype="multipart/form-data">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                            <div class="form-group">
                                <label for="name">{{__('Nom de la sous catégorie')}}</label>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="category_id" name="category_id">
                                    <option>{{ __('Sélectionnez une catégorie') }}</option>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{__('Mettre à jour la sous-catégorie')}}</h5>
                ...
                <form action="{{route('admin.products.subcategory.update')}}"  method="post">
                    ...
                    <div class="modal-body">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                        <div class="form-group">
                            <label for="edit_name">{{__('Nom de la sous-catégorie')}}</label>
                            <input type="text" class="form-control"  id="edit_name" name="title" placeholder="{{__('Nom de la sous-catégorie')}}">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
							<label for="image">Image</label>
								...
								<button type="button" class="btn btn-info media_upload_form_btn" data-btntitle="Sélectionner une image" data-modaltitle="Télécharger une image" data-imgid="" data-toggle="modal" data-target="#media_upload_modal">
...
<script>
    ;(function () {
        ...
        $(document).on('click','#media_library_image_delete_btn',function (e) {
            ...
            Swal.fire({
                title: 'Es-tu sûr?',
                text: 'Vous ne seriez pas en mesure de revenir sur cet article !',
                icon: 'warning',
        ...
        Dropzone.options.placeholderfForm = {
            dictDefaultMessage: "Faites glisser ou sélectionnez votre image",
            maxFiles: 50,
            maxFilesize: 10, //MB
            acceptedFiles: 'image/*',
    ...
    $(document).ready(function() {
        $(document).on('click', '#bulk_delete_btn', function(e) {
            ...
            if (allIds != '' && bulkOption == 'delete') {
                $(this).text('En cours de suppression...');
                $.ajax({
                    'type': "POST",
                    'url': "{{ url('/') }}/fr/ub7qfzTBzX8JXdr8V4kV7sq/products/sub-categories/bulk-action",
                    'data': {
                        _token: "{{ csrf_token() }}",
                        ids: allIds,
                    },
