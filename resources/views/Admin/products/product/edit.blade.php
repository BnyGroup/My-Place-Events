...
@endif
            ...
            <div class="col-lg-12">
                <div class="text-right mb-5">
                    <a href="{{ route('admin.products.all') }}" class="btn btn-primary">{{ __('Tous les Produits') }}</a>
                </div>
                <form action="{{ route('admin.products.update', $product->id) }}" method="POST">
                    ...
                    <div class="row mt-3">
                        <div class="col-md-8">
                            <div class="card">
                                <div class="card-body p-5">
                                    <h5 class="mb-5">{{ __('Information sur le produit') }}</h5>
                                        ...
                                        @if(is_array($all_attributes))
                                            @forelse ($all_attributes as $attribute_id => $attributes)
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col">
                                                        <div class="form-group">
                                                            <label for="">{{ __('Montant du prix additionnel') }}</label>
                                                ...
                                                @endforeach
                                            @empty
                                                <span id="no_attributes">{{ __('Aucun attribut') }}</span> 
                                            @endforelse
                                        @endif
                                    </div>
                                </div>
                        ...
                        <div class="col-md-4">
                            <div class="card mb-3">
                                <div class="card-body">
                                    <h5 class="mb-5 mt-3">{{ __('Informations sur le stock') }}</h5>
                                    <div class="form-group">
                            ...
                            <div class="card">
                                <div class="card-body">
							     ...
							     foreach ($all_attribute as $attribute){ $attribute=json_decode($attribute)[0]; ?>
                                    ...
                                    <?php /* <div class="form-group">
                                        ...
                                        @else
                                        <select class="form-control nice-select wide" name="sub_category_id[]" id="sub_category_id" multiple>
                                            <option value="">{{ __("Sélectionnez la sous-catégorie") }}</option>
                                            @foreach ($all_sub_category as $subcategory)
                                            <option value="{{ $subcategory->id }}">{{ $subcategory->title }}</option>
                                            @endforeach
                                        </select>
                                        @endif
                                        <span class="text-secondary">({{ __('Presse ') }} <kbd>{{ __('Ctrl') }}</kbd> {{ __(' et Cliquez pour sélectionner plusieurs options') }})</span>
                                    </div>*/ ?>
                                    <div class="form-group">
                                            <label for="image">Image</label>
                                                <div class="media-upload-btn-wrapper">
                                                <div class="img-wrap">
                                                        <?php $imgUrl=get_image_url($product->image, $size = 'full'); ?>
                                                        <div class="rmv-span"><i class="fa fa-trash"></i></div>
                                                        <div class="attachment-preview">
                                                            <div class="thumbnail">
                                                                <div class="centered">
                                                                    <img class="avatar user-thumb" src="{{$imgUrl}}" >
                                                                </div>
                                                            </div>
                                                        </div>
                                                </div>  
                                                 <br>
                                                <input type="hidden" name="image" value="{{$product->image}}"data-imgsrc="{{$imgUrl}}">
                                                <button type="button" class="btn btn-info media_upload_form_btn2" 
						data-btntitle="Select Image" data-modaltitle="Upload Image" data-imgid="674" 
						data-toggle="modal" data-target="#media_upload_modal2">
                                                    Changer l'image
                                                </button>
                                            </div>
                                            <small>La taille d'image recommandée est de 1280 x 1280px.</small>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="status" id="status">
                                            <option value="draft" @if($product->status == 'draft') selected @endif>{{ __('Brouillon') }}</option>
                                            <option value="publish" @if($product->status == 'publish') selected @endif>{{ __('Publier') }}</option>
                                        </select>
                                    </div>
                                    <div class="text-center mt-5">
                                        <button class="btn btn-primary">{{ __('Mettre à jour le produit') }}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="upload_files" role="tabpanel" >
                        <div class="dropzone-form-wrapper">
                            <form action="{{ url('/') }}/fr/ub7qfzTBzX8JXdr8V4kV7sq/media-upload" method="post" id="placeholderfForm" class="dropzone" enctype="multipart/form-data">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            </form>
                        </div>
                    </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary media_upload_modal_submit_btn" style="display: none">Sélectionner Image</button>
            </div>
        </div>
    </div>
</div>
...
$('#category_id').on('change', function () {
    ...
    ;(function () {
        ...
        $(document).on('click','.media_upload_modal_submit_btn',function (e) {
            ...
            $('.media_upload_form_btn').text('Changer l\'image');
        });
