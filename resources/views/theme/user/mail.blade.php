...
<head>
	...
	<title>Activer votre compte sur {{ forcompany() }}</title>
	<style type="text/css">
		@font-face {
		...
		<div class="mail-inner-content">
			...
			<div class="mail-header">
				...
				<div class="mail-header-title" >
					<h1>Activez votre compte sur {{ forcompany() }}</h1>
				</div>
			</div>
			<div class="mail-body">
				<div class="body-title">
					<h4>
						Salut  <span>{{ $userdata->firstname }} {{ $userdata->lastname }}</span>,
					</h4>
					<p>Vous avez presque terminé ! Veuillez cliquer sur ce lien pour activer votre compte et vous connecter avec votre mot de passe.</p>

					<a href="{{ route('activation',$userdata->remember_token) }}" class="mail-button">{{--Activate Account--}} Activer mon compte</a>

					<p>Si vous n'avez pas essayé récemment d'activer votre compte sur {{ forcompany() }}, ignorez ce mail. </p>
				</div>
				<div class="bottom-dashed"></div>
				<p class="term-service">
					Soumis aux <a href="https://blog.myplace-events.com/condition-generales-dutilisation/" target="_blank" >Conditions générales d'utilisation</a> de {{ forcompany() }} .
				</p>
			</div>
			<!-- FOOTER -->
			<div class="mail-footer">
				<p> Cet email a été envoyé depuis {{ frommail() }} <br/>
				 Copyright © {{ date('Y') }} {{ forcompany() }}. Tous droits réservés. </p>
			</div>
