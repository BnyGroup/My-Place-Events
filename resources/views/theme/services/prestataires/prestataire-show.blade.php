@extends($theme)
@section('meta_title',/*setMetaData()->org_single_title.*/' | Prestataire - '.$user->pseudo )
@section('meta_description',/*setMetaData()->org_single_desc*/ 'Profil du prestataire')
@section('meta_keywords',/*setMetaData()->org_single_keyword*/$user->pseudo)
@section('content')
...
<style>
	...
	section.page-title{
		padding: 20px 0 20px 0; width: 100%; margin-top: -50px; position: fixed
	}

</style>
 
<div class="main-container" id="single-artistes">
	...
	<section class="page-title page-title- fil-ariane-light">
		<div class="container">
			<div class="row">
				...
				<div class="breadcrumb breadcrumb-2 text-left">
					<p id="breadcrumbs"><span><span><a href="{{ url('/') }}{{--https://myplace-event.com/--}}">@lang('words.nav_bar.nav_bar_menu_1')</a> / <span><a href="{{--https://myplace-event.com/events/--}}{{ url('prestataire') }}">Prestataire</a> / <strong class="breadcrumb_last primary-color" aria-current="page">{{ $data->pseudo }}</strong></span></span></span></p>
				</div>
		...
		<!--end of container-->
			<!--Cover-->
			<div class="col-md-12 cover-img" style="background-image:url('{{ url($data->cover) }}'); "></div>
			<!--Cover-->

			<div class="oinfos">
				<div style="width: 100%">
					<h1 class="min-title mb-4">{{ $data->firstname }} {{ $data->lastname }}</h1>
				</div>
				...
				<div style="width: 100%; min-height: 100px; margin: auto; margin-top:15px;">
					<a class="prestBut" href="">{{ $catdata[0]->category_name }}</a>
					<a class="prestButCont" href="javscript:void(0)"  data-toggle="modal" data-target="#contact-org">Contacter le Prestataire</a>
				</div>
			</div>
        ...
        <div class="container" style="padding-bottom: 50px;margin-top: 50px;">
            <div class="row">
				<!--Photo de profil-->
				<div class="col-lg-12 col-sm-12 col-xs-12 text-center Info2b">
					<div class="row">
						<div class="profile">
							<img src="{{ setThumbnail($data->profile_pic) }}" class="img-fluid">
						</div>
					...
					<div class="modal-dialog modal-xs">
						<div class="modal-content ticket-registion">
							<div class="modal-header">
								<h5 class="modal-title" id="exampleModalLabel">Contacter le prestataire <strong class="primary-color">{{ $data->pseudo }}</strong></h5>
								<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
							</div>
							<div class="modal-body">
								<form id="org-con-form" method="post" action="{{ route('pre.contact') }}">
									{!! csrf_field() !!}
									<div class="form-group row">
										<label for="staticEmail" class="col-sm-2 col-sm-offset-4 col-form-label">Nom</label>
										<div class="col-sm-10">
											<input type="hidden" name="org_mail" value="{{--{{ $data->adresse_mail }}--}}jeanblanchardbom@gmail.com">
									...
									<input type="submit" id="organizer-form" class="btn btn-flat btn-primary btn-block" value="Envoyer le message">
								</form>
							</div>
            ...
            <div class="row ephotos">
				
				<div class="left-info info-box col-lg-12 ">
					<div class="col-lg-12 text-center content-title">
					   <h1 style="margin-bottom: 0;" class="section-title">Mes Réalisations</h1>
					</div>
					...
					<div class="col-lg-12">
						...
						@if(!empty($realisations))
							<div id="aniimated-thumbnials" class="justified-gallery row">
								@foreach($realisations as $realisation)
									<?php /*<div class="col-lg-4 inboxx">*/ ?>
										<a class="col-lg-4 inboxx" href="{{ getImage($realisation->realisation) }}">
											<img src="{{ getImage($realisation->realisation) }}"/>
										</a>
									<?php /*</div>*/ ?>
								@endforeach
...
<style>
	.inboxx img{
		width: 100%
                ...
                <div class="col-lg-12">
					...
					<div class="row">
						<div class="col-lg-4">
								<div class="col-lg-12 text-center content-title" style="margin-left: 0px; padding-left: 0px;">
								   <h1 style="margin-bottom: 0; font-size: 23px;" class="section-title infocontact">Infos du Prestataire</h1>
								</div>
								<ul style="list-style: none;">
									<li><strong class="primary-color">Tel :</strong>
										<a class="third-color " href="tel:{{ $data->indicatif_1 }}{{ $data->telephone_1 }}">
											<strong>
												{{ $data->indicatif_1 }} {{ $data->telephone_1  }}
											</strong>
											<br>
										</a>
									</li>

									<li><strong class="primary-color">Email :</strong>
										<a class="third-color" href="mailto:{{ $data->adresse_mail }}"><strong>{{ $data->adresse_mail }}</strong></a>
									</li>

									<li><strong class="primary-color">Lieu :</strong>
										<a class="third-color" href=""><strong> {{ $data->adresse_geo }}</strong></a>
									</li>

									<?php /*?><li> 
										<a class="prestBut" href="" style="display: inline-block">{{ $data->activites }} </a>				
									</li> <?php */?>                        
								</ul>
						...
						<div class="col-lg-4" style="text-align: right">
							<h1 class="socialTitle">SUIVEZ-MOI<br>SUR LES RESEAUX</h1>
							<?php if(!empty($data->facebook)){ ?><a href="{{ $data->facebook }}" class="btn btn-sm btn-rds fbx" style="border-radius: 50% !important; display: inline-block" target="_blank" ><i class="fab fa-facebook-f"></i></a><?php } ?>
							<?php if(!empty($data->instagram)){ ?><a href="{{ $data->instagram }}" class="btn btn-sm btn-rds instax" style="border-radius: 50% !important; display: inline-block" target="_blank" ><i class="fab fa-instagram"></i></a><?php } ?>

							<?php if(!empty($data->twitter)){ ?><a href="{{ $data->twitter }}" class="btn btn-sm btn-rds twtx" style="border-radius: 50% !important; display: inline-block" target="_blank" ><i class="fab fa-twitter"></i></a><?php } ?>
	
							<?php if(!empty($data->youtube)){ ?><a href="{{ $data->youtube }}" class="btn btn-sm btn-rds ytx" style="border-radius: 50% !important; display: inline-block" target="_blank" ><i class="fab fa-youtube"></i></a><?php } ?>
						</div>
					</div>
                </div>
                <!--Infos contacts-->
    ...
    <section class="prestataire-section" {{--style="display:none"--}}>
                            ...
                            <div class="col-md-9 col-sm-12">
                                <div class="tab-content" id="v-pills-tabContent" style="font-size: medium">
                                    <div class="tab-pane fade show active" id="v-pills-shooting" role="tabpanel" aria-labelledby="v-pills-home-tab" >
                                        ...
                                        <form method="post" action="{{ route('pre.demande') }}">
                                            {!! csrf_field() !!}
                                            ...
                                            <input type="submit" class="btn btn-flat btn-primary btn-block" value="Demander une séance de Shooting">
                                        </form>
                                    </div>
                                    ...
                                    <div class="tab-pane fade" id="v-pills-capsules" role="tabpanel" aria-labelledby="v-pills-profile-tab">
                                        ...
                                        <form method="post" action="{{ route('pre.demande') }}">
                                            ...
                                            <input type="submit" class="btn btn-flat btn-primary btn-block" value="Demander une Capsule Vidéo">
                                        </form>
                                    </div>
                                        ...
                                        <form method="post" action="{{ route('pre.demande') }}">
                                            ...
                                            <div style=" text-align: left;">
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-sm-10">
                                                        <input type="checkbox" name="service[]" value="Création Graphique" id="creation-graphique"> <label for="creation-graphique">: Création Graphique </label>&nbsp; <br><input type="checkbox" name="service[]" value="Rédaction" id="redaction"> <label for="redaction">: Rédaction </label>
                                                    </div>
                                                ...
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-sm-10">
                                                        <input type="checkbox" name="reseau[]" value="Facebook" id="facebook"> : <label for="facebook"> <i class="fab fa-facebook"></i> Facebook</label><br><input type="checkbox" name="reseau[]" value="Instagram" id="instagram"> :<label for="instagram"> <i class="fab fa-instagram"></i> Instagram </label><br>
                                                        <input type="checkbox" name="reseau[]" value="Twitter" id="twitter"> : <label for="twitter"> <i class="fab fa-twitter"></i>Twitter</label> <br> <input type="checkbox" name="reseau[]" value="Whatsapp" id="whatsapp"> : <label for="whatsapp"><i class="fab fa-whatsapp"></i> Whatsapp </label><br>
                                            ...
                                            <input type="submit" class="btn btn-flat btn-primary btn-block" value="Demander l'animation de mes pages">
                                        </form>
                                    </div>
                                    ...
                                    <div class="tab-pane fade" id="v-pills-clip" role="tabpanel" aria-labelledby="v-pills-settings-tab">
                                        ...
                                        <form method="post" action="{{ route('pre.demande') }}">
                                            ...
                                            <input type="submit" class="btn btn-flat btn-primary btn-block" value="Demander un clip musical">
                                        </form>
                                    </div>
                                    ...
                                    <div class="tab-pane fade" id="v-pills-placement" role="tabpanel" aria-labelledby="v-pills-settings-tab">
                                        ...
                                        <form method="post" action="{{ route('pre.demande') }}">
                                            ...
                                            <input type="submit" class="btn btn-flat btn-primary btn-block" value="Demander un placement d'événement">
                                        </form>
