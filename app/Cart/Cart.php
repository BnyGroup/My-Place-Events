...
class Cart
{
	...
	/**
	 * Get the total of all item's quantity in cart.
	 *
	 * @return int
	 */
	...
	/**
	 * Get the total of an item's quantity in cart.
	 *
	 * @return int
	 */
	...
	/**
	 * Get the total of an item's total price in cart.
	 *
	 * @return int
	 */
	public function getItemPrice($id)
	{
		...
		if (isset($this->items[$id])) {
			foreach ($this->items[$id] as $item) {
				$price += isset($item['attributes']['price']) ? $item['attributes']['price'] * $item['quantity'] : 0;
			}
		}
		return $price;
	}
