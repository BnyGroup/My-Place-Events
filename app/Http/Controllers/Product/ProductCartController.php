...
class ProductCartController extends Controller
{
    ...
    public function getCartInfoAjax()
    {
        ...
        return [
            ...
            'cart' => view('theme.gadgets.mini-cart', compact('all_cart_items', 'products', 'subtotal'))->render()
        ];
    }
    ...
    public function addToCartAjax(Request $request)
    {
        ...
        if ($campaign['campaign_quantity'] > -2) {  
        ...
        if ($remaining_quantity) { 
			...
			if($remaining_quantity<0){
				$vcat = new Cart();
				$tt=$vcat->getItemQuantity($request->product_id);
				$tt=$tt-1;  
			} 
            ...
            if ($available_quantity > 0) {
				...
				$vcat = new Cart();
				$vcat->update($request->product_id, $tt);	
			
			}
        }

        $response_data = FlashMsg::explain('success', __('Article ajouté au panier'));
        $response_data['cart_info'] = CartAction::getCartInfo();

        if ($available_quantity != $remaining_quantity) {
            $response_data['quantity_msg'] = __('La quantité demandée n\'est pas disponible. Seule la quantité disponible est ajoutée au panier');
        }
    ...
    public function updateCart(Request $request)
    {
        ...
        return view('theme.gadgets.cart.cart-partial', compact('all_cart_items', 'products', 'subtotal', 'default_shipping_cost', 'total', 'quantity_msg'));
    }
    ...
    public function removeCartItem(Request $request)
    {
        ...
        return view('theme.gadgets.cart.cart-partial', compact('all_cart_items', 'products', 'subtotal', 'default_shipping_cost', 'total'));
    }
